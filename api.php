...
function pidExists($pid){
    ...
    $cmd = 'powershell -NoProfile -ExecutionPolicy Bypass -Command "Get-Process -Id ' . $pid . ' -ErrorAction SilentlyContinue"';
...
// =====================
// Inicio servidor oculto con PID
// =====================
function startServerHidden($exe, $params){
    if (!is_file($exe)) return false;

    $workDir = dirname($exe);

    // 💡 Iniciar usando popen (ventana oculta automatizada)
    $cmd = "start \"VHSERVER\" /B \"" . $exe . "\" " . $params;
    pclose(popen($cmd, "r"));

    // Esperar un poco a que Windows registre el proceso
    sleep(2);

    // Capturar PID por búsqueda de proceso activo con nombre del exe
    $ps = 'powershell -NoProfile -ExecutionPolicy Bypass -Command '
        . '"Get-Process -Name valheim* | Select-Object -ExpandProperty Id"';

    $pidList = trim(shell_exec($ps));

    if (!$pidList) return false;

    // Tomar el último PID listado
    $pidArray = preg_split("/\s+/", $pidList);
    $pid = intval(end($pidArray));

    return $pid > 0 ? $pid : false;
}

// =====================
// Servidor: iniciar / detener / estado
// =====================
$action = $_GET['action'] ?? '';
...
case 'delete_backup': {
    ...
    // === Obtener configuraciones de servers.json
    case 'get_servers_json':
        if (!defined('SERVERS_JSON') || !file_exists(SERVERS_JSON))
            out(['ok'=>false,'error'=>'No existe servers.json']);
        out(['ok'=>true,'content'=>file_get_contents(SERVERS_JSON)]);

    
    // === Iniciar/Detener Servidor
    case 'server':
        $op = $_POST['op'] ?? '';
        $servers = file_exists(SERVERS_JSON)
            ? json_decode(file_get_contents(SERVERS_JSON), true)
            : [];

        if (!$servers || !isset($servers[0]))
            out(['ok'=>false,'error'=>'servers.json vacío o inválido']);

        $srv = $servers[0];
        $exe = $srv['path'] ?? '';
        $params = $srv['params'] ?? '';

        if (!is_file($exe)) out(['ok'=>false,'error'=>'Ejecutable no encontrado']);

        if ($op === 'start'){

            if (isServerRunning())
                out(['ok'=>false,'error'=>'Servidor ya está iniciado.']);

            $pid = startServerHidden($exe, $params);

            if (!$pid)
                out(['ok'=>false,'error'=>'No se pudo obtener PID del proceso.']);

            savePID($pid);
            out(['ok'=>true,'msg'=>"Servidor iniciado con PID $pid"]);
        }

        if ($op === 'stop'){
            if (!isServerRunning())
                out(['ok'=>false,'error'=>'Servidor no está ejecutándose.']);

            $pid = getSavedPID();
            shell_exec('powershell -NoProfile -ExecutionPolicy Bypass -Command "Stop-Process -Id ' . $pid . ' -Force"');
            sleep(2);

            if(!pidExists($pid)){
                clearPID();
                out(['ok'=>true,'msg'=>"Servidor detenido (PID $pid)"]);
            }

            out(['ok'=>false,'error'=>'Error deteniendo el proceso']);
        }

        out(['ok'=>false,'error'=>'Operación inválida']);
    ...
    case 'status_servers':
        out([
            'ok'=>true,
            'status'=>[
                'current' => isServerRunning() ? 'running' : 'stopped'
            ]
        ]);
